...
int32_T SparkSeqDiag_23_Count = 0;
int32_T SparkSeqDiag_23_LastPin = -1;
uint8_T SparkSeqDiag_23_Short[8];
uint8_T SparkSeqDiag_23_Open[8];
uint16_T SparkSeqDiag_23_OpenADC[8];
uint16_T SparkSeqDiag_23_OnTime[8];
void SparkSeqDiagCallback_23(S_SeqOutDiagReportPtr report, NativePtrSizedInt_U
  userData)
{
  ...
  if (report->DiagReportObj.EST.u1ValidDataMask & HAS_SHORT_CCT_EST_DIAG) {
    SparkSeqDiag_23_OnTime[pin_number] =
      report->DiagReportObj.EST.ShortDataObj.u2OnTimeInMicroSecs;
    if (report->DiagReportObj.EST.ShortDataObj.b1Occurred == 0) {
      SparkSeqDiag_23_Short[pin_number] = 1;/* Fault */
    } else {
      SparkSeqDiag_23_Short[pin_number] = 0;/* No Fault */
    }
  } else {
    SparkSeqDiag_23_Short[pin_number] = 2;/* Indeterminate */
  }

  /* Check for open circuit */
  if (report->DiagReportObj.EST.u1ValidDataMask & HAS_OPEN_CCT_EST_DIAG) {
    SparkSeqDiag_23_OpenADC[pin_number] =
      report->DiagReportObj.EST.OpenDataObj.uADCSample;
    if (report->DiagReportObj.EST.OpenDataObj.uADCSample >= 1023) {
      SparkSeqDiag_23_Short[pin_number] = 1;/* Short to Power */
      SparkSeqDiag_23_Open[pin_number] = 0;/* Not Open */
    } else if (report->DiagReportObj.EST.OpenDataObj.uADCSample <= 10) {
      SparkSeqDiag_23_Short[pin_number] = 1;/* Short to Ground */
      SparkSeqDiag_23_Open[pin_number] = 0;/* Not Open */
    } else if (report->DiagReportObj.EST.OpenDataObj.uADCSample >
               (EST_OpenThreshold_DataStore())) {
      SparkSeqDiag_23_Short[pin_number] = 0;/* Not Shorted */
      SparkSeqDiag_23_Open[pin_number] = 1;/* Open Fault */
    } else {
      SparkSeqDiag_23_Short[pin_number] = 0;/* Not Shorted */
      SparkSeqDiag_23_Open[pin_number] = 0;/* Not Open */
    }
  } else {
    SparkSeqDiag_23_Open[pin_number] = 2;/* Indeterminate */
  }

  SparkSeqDiag_23_Count++;
  SparkSeqDiag_23_LastPin = pin_number;
}

void ReSyncAfterCrankUnsync_23(void)
{
