...
int8_T TableInterpolation2D_int8_T(uint16_T row_in, uint16_T col_in, int8_T
  *map_data, uint32_T row_sz, uint32_T col_sz)
{
  int8_T out;
  int32_T result;
  ...
  result += (col_frac - q_frac) * r0c1_value;
  result += (q_frac) * r1c1_value;
  out = (int8_T)((result + 256) >> 9);
  return out;
}

int8_T TableInterpolation1D_int8_T(uint16_T idx, int8_T *tbl_data, uint32_T sz)
{
  int8_T out;
  int32_T range;
  int8_T lo_value;
  ...
  if (ord >= sz-1) {
    ...
    range = (int32_T)hi_value - (int32_T)lo_value;
    frac = (idx) & 0x1FF;
    out = (int8_T)((int32_T)lo_value + (((int32_T)frac * range) >> 9));
  }
