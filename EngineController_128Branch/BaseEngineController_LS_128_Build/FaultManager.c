...
void RefreshFaultActionReferenceCounts(void)
{
  ...
  uint16_T action;
  uint16_T index;
  for (action=0; action < 15; action++) {
    Actions_DataStore_ByIndex(action).reference_count = 0;
  }

  for (fault=0; fault < 162; fault++) {
    uint8_T mask;
    mask = (uint8_T) (1U << (fault & 7U));
    index = (uint16_T) (fault >> 3UL);
...
void GlobalFaultClear(void)
{
  ...
  FaultMarquees_DataStore_FaultMarquees().current_suspected_vardec = 162;
  FaultMarquees_DataStore_FaultMarquees().current_active_vardec = 162;
  FaultMarquees_DataStore_FaultMarquees().current_occurred_vardec = 162;
  VolFaultManager_DataStore_VolFaultManager().last_suspected_fault = 162;
  VolFaultManager_DataStore_VolFaultManager().last_unsuspected_fault = 162;
  VolFaultManager_DataStore_VolFaultManager().last_active_fault = 162;
  VolFaultManager_DataStore_VolFaultManager().last_inactive_fault = 162;
  VolFaultManager_DataStore_VolFaultManager().last_occurred_fault = 162;
}
...
static void UpdateActionReasonMarquee(uint32_T fault_action_index)
{
  ...
  static fault_action_marquee_state_t fault_action_marquee_state[15] = { { 0 } };
  ...
  (FaultActionReason_DataStore())[fault_action_index] = 162;
}
