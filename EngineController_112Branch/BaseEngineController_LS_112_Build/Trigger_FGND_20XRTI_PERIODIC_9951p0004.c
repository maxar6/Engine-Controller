...
#define BaseEngineController_LS_112_IN_PowerUp (2U)
  ...
  if (BaseEngineController_LS_112_DWork.s689_motohawk_trigger_DWORK1) {
    /* Output and update for function-call system: '<S689>/Function-Call Subsystem' */
    {
      boolean_T rtb_InitialConditionisTrue_f;

      /* UnitDelay: '<S707>/Initial Condition is True' */
      rtb_InitialConditionisTrue_f =
        BaseEngineController_LS_112_DWork.s707_InitialConditionisTrue_DSTATE;

      /* Logic: '<S706>/Logical Operator' incorporates:
       *  S-Function (motohawk_sfun_calibration): '<S706>/motohawk_calibration'
       */
      BaseEngineController_LS_112_B.s706_LogicalOperator =
        ((rtb_InitialConditionisTrue_f || (APP1_AlwaysAdapt_DataStore())));

      /* Update for UnitDelay: '<S707>/Initial Condition is True' incorporates:
       *  Constant: '<S707>/False After First Sample'
       */
      BaseEngineController_LS_112_DWork.s707_InitialConditionisTrue_DSTATE =
        FALSE;
    }
