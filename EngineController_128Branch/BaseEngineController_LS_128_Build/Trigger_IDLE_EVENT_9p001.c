...
#define BaseEngineController_LS_128_IN_PowerUp (2U)
  ...
  if (BaseEngineController_LS_128_DWork.s1_motohawk_trigger1_DWORK1) {
    /* Output and update for function-call system: '<S4>/CCP Command Driver' */
    {
      ...
      do {
        ...
        /* MotoHawk Read CAN Raw */
        {
          ...
          BaseEngineController_LS_128_B.s9_ReadCANRaw_o1 = retval;
          if ((BaseEngineController_LS_128_B.s9_ReadCANRaw_o2 + 1) >
              BaseEngineController_LS_128_B.s9_ReadCANRaw_o2)
            BaseEngineController_LS_128_B.s9_ReadCANRaw_o2++;
          if (retval) {
            BaseEngineController_LS_128_B.s9_ReadCANRaw_o3 = ((messageObj.eFlags
              & CAN_MESSAGE_EXTENDED) != 0);
            BaseEngineController_LS_128_B.s9_ReadCANRaw_o4 =
              messageObj.u4MessageID;
            if (messageObj.u1Length > 8) {
              messageObj.u1Length = 8;
            }

            BaseEngineController_LS_128_B.s9_ReadCANRaw_o5 = messageObj.u1Length;
            for (i=0; i < messageObj.u1Length; i++) {
              BaseEngineController_LS_128_B.s9_ReadCANRaw_o6[i] =
                messageObj.u1DataArr[i];
            }

            BaseEngineController_LS_128_B.s9_ReadCANRaw_o2 = 0;
          }
        }
        ...
        /* <S9>/motohawk_sfun_ccp_protocol: CCP Protocol CCP1 */
        {
          ...
          /* Request */
          if (BaseEngineController_LS_128_B.s9_ReadCANRaw_o1) {
            if (BaseEngineController_LS_128_B.s10_motohawk_calibration3 !=
            ...
            mh_CCP1_CCPHandlerInstance.id =
              BaseEngineController_LS_128_B.s10_motohawk_calibration;
            mh_CCP1_CCPHandlerInstance.ext = TRUE;
            mh_ProcessCCP(BaseEngineController_LS_128_B.s9_ReadCANRaw_o5,
                          BaseEngineController_LS_128_B.s9_ReadCANRaw_o6,
                          &mh_CCP1_CCPHandlerConfig, &mh_CCP1_CCPHandlerInstance);
          }
        }

        s9_iter++;
      } while (BaseEngineController_LS_128_B.s9_ReadCANRaw_o1 && (s9_iter <= 5));
