				...
				@copy "LinkerDefinition_DEV.xml" "LinkerDefinition.xml" > NUL
					...
					@"%MOTOCODER_DIR%\bin\Transform" -o TDB/ParsedAndCombinedVardecs.xml TDB/ParsedAndCCVardecs.xml "%MOTOCODER_DIR%\Transforms\VarDecs_Merge.xsl" AdditionalVardecsXML="%cd%\MotoCoderVarDecs.xml"
							@if ERRORLEVEL 1 exit junk
					@"%MOTOCODER_DIR%\bin\Transform" -o TDB/BaseEngin_026.xml TDB/ParsedAndCombinedVardecs.xml "%MOTOCODER_DIR%\Transforms\VarDecs_Reorder.xsl" application-url="%CD%\ApplicationDescriptor.xml" maxTableSize=65535
							@if ERRORLEVEL 1 exit junk
					@"%MOTOCODER_DIR%\bin\Transform" TDB/BaseEngin_026.xml "%MOTOCODER_DIR%\Transforms\VarDecs_Transform.xsl" includeFiles="CommonInclude.h,BaseEngineController_LS.h,TDB_Includes.h" GenDLL=1 GenTDB=1
							@if ERRORLEVEL 1 exit junk
					@"%MOTOCODER_DIR%\bin\nant\bin\nant.exe" -buildfile:TDB\Database.build -D:required.installdir="C:\Program Files (x86)\Woodward\DevelopmentTools\Toolchains\GCC\win32-pe\4_4_0" -D:database.basename=BaseEngin_026 -q -nologo rebuild
							@if ERRORLEVEL 1 exit junk
					...
					@echo ### Copy BaseEngin_026.dll and BaseEngineController_LS_026.srz
					@if exist c:\ecufiles\tdbdll\*.* copy /Y .\TDB\BaseEngin_026.dll c:\ecufiles\tdbdll > NUL
							@if ERRORLEVEL 1 exit junk
					@if exist C:\ECUFiles\Programs\\*.* copy /Y .\Target\BaseEngineController_LS_026.srz C:\ECUFiles\Programs\ > NUL
							@if ERRORLEVEL 1 exit junk
				...
				@if exist "%MOTOCODER_DIR%\bin\ESTconvert.exe" "%MOTOCODER_DIR%\bin\ESTconvert.exe" .\Target\BaseEngineController_LS.elf -b -c > NUL 2>&1
					...
					@"C:\PROGRA~2\Woodward\MCS\MotoHawk\2011A_~1.184\Bin\motohawk_dumpdll.exe" -xml "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\TDB\BaseEngin_026.dll" > "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\TDB\BaseEngin_026.xml"
							@if ERRORLEVEL 1 exit junk
					
				
					@echo ### Generating ASAP2 file
					@"C:\PROGRA~2\Woodward\MCS\MotoHawk\2011A_~1.184\Bin\ASAP2.exe" -m "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\TDB\BaseEngin_026.xml" -o "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\TDB\BaseEngin_026.a2l" -u VirtualCalSpace -asap1b "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\mh_asap1b_ccp_CCP1.xml"
							@if ERRORLEVEL 1 exit junk
					
				
					@echo ### Installing ASAP2 file in c:\ecufiles\tdbdll
					@copy "C:\Users\maxar\Documents\GitHub\Engine-Controller\EngineController_Rev2\BaseEngineController_LS_Build\TDB\BaseEngin_026.a2l" "c:\ecufiles\tdbdll" > NUL
							@if ERRORLEVEL 1 exit junk
